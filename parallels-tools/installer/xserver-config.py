...
def print_prl_section(fout, sname, key, sextra = []):
	...
	print >> fout, "\n# %s section" % prl_ids[key]
	print >> fout, "Section \"%s\"" % sname
	print >> fout, "\tIdentifier\t\"%s\"" % prl_ids[key]

	# Print out section specific information
	if key == "mouse":
		if prl_mdevice == "":
			prl_mdevice = prl_ddevice
		print >> fout, "\tDriver\t\"prlmouse\""
		print >> fout, "\tOption\t\"Device\"\t\"%s\"" % prl_mdevice
		print >> fout, "\tOption\t\"Protocol\"\t\"auto\""
	elif key == "video":
		print >> fout, "\tDriver\t\"prlvideo\""
	elif key == "monitor":
		print >> fout, "\tVendorName\t\"Parallels Inc.\""
		print >> fout, "\tModelName\t\"%s\"" % prl_ids["monitor"]
	elif key == "screen":
		print >> fout, "\tDevice\t\"%s\"" % prl_ids["video"]
		print >> fout, "\tMonitor\t\"%s\"" % prl_ids["monitor"]
		print >> fout, "\tOption\t\"NoMTRR\""

	# Print out extra section information
	for line in sextra:
		print >> fout, line,

	print >> fout, "EndSection"
...
def print_extra_section(fout, sname, idname, sextra = [], add_aei = False):
	...
	if lsname == "serverlayout":
		print >> fout, "\tIdentifier\t\"%s\"" % idname
		if is_vtd_video_enabled():
			print >> fout, "\t#Screen\t\"%s\"" % prl_ids["screen"]
		...
		if idname == prl_dmid:
			print >> fout, "\tIdentifier\t\"%s\"" % prl_dmid
			print >> fout, "\tDriver\t\"%s\"" % prl_dmdrv
		elif idname == prl_dkid:
			print >> fout, "\tIdentifier\t\"%s\"" % prl_dkid
			print >> fout, "\tDriver\t\"%s\"" % prl_dkdrv
			print >> fout, "\tOption\t\"XkbModel\"\t\"pc105\""
			print >> fout, "\tOption\t\"XkbLayout\"\t\"us\""
			print >> fout, "\tOption\t\"XkbOptions\"\t\"grp:alt_shift_toggle\""
	elif lsname == "serverflags" and add_aei:
		print >> fout, "\tOption\t\"AllowEmptyInput\"\t\"yes\""
		print >> fout, "\tOption\t\"AutoAddDevices\"\t\"yes\""
...
def configure_xorg(xtype, xversion, fin, fout):
	...
	# Add header
	print >> fout, "#"
	print >> fout, "# X.Org server configuration file generated by Parallels Guest Tools."
	print >> fout, "#"
...
def main():
	...
	try:
		...
		print "Error: exception occured during configuration of X server"
		print "       ", why
